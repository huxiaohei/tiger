...
void test_iface()
{
    ...
    for (auto it : rts)
    {
        TIGER_LOG_D(tiger::TEST_LOG) << "[name:" << it.first << " ip:" << it.second.first->to_string()
                                     << " prefixlen:" << it.second.second
                                     << " subnetmask:" << it.second.first->subnet_mask(it.second.second)->to_string()
                                     << " networkAddress:"
                                     << it.second.first->network_address(it.second.second)->to_string()
                                     << " broadcastAddress:"
                                     << it.second.first->broadcast_address(it.second.second)->to_string() << "]";
    }
    ...
    for (auto it : lo_rts)
    {
        TIGER_LOG_D(tiger::TEST_LOG) << "[name:lo"
                                     << " ip:" << it.first->to_string() << " prefixlen:" << it.second
