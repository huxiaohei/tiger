...
class ZlibStream : public Stream
{
  ...
  private:
    int init(Type type = Type::DEFLATE, CompressLevel level = CompressLevel::DEFAULT_COMPRESSION, int window_bits = 15,
             int mem_level = 8, Strategy strategy = Strategy::DEFAULT);
  ...
  public:
    ...
    static ZlibStream::ptr Create(bool encode, uint32_t buff_szie = 4096, Type type = Type::DEFLATE,
                                  CompressLevel level = CompressLevel::DEFAULT_COMPRESSION, int window_bits = 15,
                                  int mem_level = 8, Strategy strategy = Strategy::DEFAULT);
  ...
  public:
    ...
    void set_encode(bool v)
    {
        m_encode = v;
    }
    std::vector<iovec> &get_buffers()
    {
        return m_buffers;
    }
