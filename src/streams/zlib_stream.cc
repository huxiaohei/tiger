...
int ZlibStream::init(Type type, CompressLevel level, int window_bits, int mem_level, Strategy strategy) {
    TIGER_ASSERT_WITH_INFO((level >= 0 && level <= 9) || level == CompressLevel::DEFAULT_COMPRESSION, "[ZlibStream init level error]");
    ...
    if (m_encode) {
        int rt = deflateInit2(&m_zstream, level, Z_DEFLATED,
                              window_bits, mem_level, (int)strategy);
        return rt;
    } else {
...
ZlibStream::ZlibStream(bool encode, uint32_t buff_size)
    : m_encode(encode), m_free(true), m_buff_size(buff_size) {
}
...
ZlibStream::ptr ZlibStream::Create(bool encode, uint32_t buff_szie,
                                   Type type, CompressLevel level,
