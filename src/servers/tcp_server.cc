...
static ConfigVar<int64_t>::ptr g_tcp_server_accept_timeout = Config::Lookup<int64_t>("tiger.servers.tcp.acceptTimeout", 6000, "tcp accept timeout");
static ConfigVar<int64_t>::ptr g_tcp_server_recv_timeout = Config::Lookup<int64_t>("tiger.servers.tcp.recvTimeout", 6000, "tcp accept timeout");
...
TCPServer::TCPServer(IOManager::ptr io_worker, IOManager::ptr accept_worker, const std::string &name)
    : m_io_worker(io_worker),
      ...
      m_accept_timeout(g_tcp_server_accept_timeout->val()),
      m_recv_timeout(g_tcp_server_recv_timeout->val()),
      m_name(name),
      m_is_stop(true) {
}
...
bool TCPServer::start() {
    ...
    for (auto &socket : m_sockets) {
        m_accept_worker->schedule(
            std::bind(&TCPServer::start_accept, shared_from_this(), socket));
    }
    return true;
}
