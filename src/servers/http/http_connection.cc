...
HTTPConnection::HTTPConnection(Socket::ptr sock, bool owner) : SocketStream(sock, owner)
{
    ...
    TIGER_LOG_I(SYSTEM_LOG) << "[connection construct"
                            << " create:" << m_create_time << " socket:" << get_socket() << "]";
}
...
HTTPResult::ptr HTTPConnection::Get(const std::string &url, uint64_t timeout,
...
HTTPResult::ptr HTTPConnection::Get(URI::ptr uri, uint64_t timeout, const std::map<std::string, std::string> &headers,
                                    const std::string &body)
{
    return Request(HTTPMethod::GET, uri, timeout, headers, body);
}

HTTPResult::ptr HTTPConnection::Post(const std::string &url, uint64_t timeout,
                                     const std::map<std::string, std::string> &headers, const std::string &body)
{
    URI::ptr uri = URI::Create(url);
    if (!uri)
...
HTTPResult::ptr HTTPConnection::Post(URI::ptr uri, uint64_t timeout, const std::map<std::string, std::string> &headers,
                                     const std::string &body)
{
    return Request(HTTPMethod::POST, uri, timeout, headers, body);
}

HTTPResult::ptr HTTPConnection::Request(HTTPMethod method, const std::string &url, uint64_t timeout,
                                        const std::map<std::string, std::string> &headers, const std::string &body)
{
    URI::ptr uri = URI::Create(url);
    if (!uri)
...
HTTPResult::ptr HTTPConnection::Request(HTTPMethod method, URI::ptr uri, uint64_t timeout,
                                        const std::map<std::string, std::string> &headers, const std::string &body)
{
...
HTTPResult::ptr HTTPConnection::Request(HTTPRequest::ptr req, URI::ptr uri, uint64_t timeout)
{
    ...
    if (!addr)
    {
        std::stringstream ss;
        ss << "[Invalid host"
           << " host:" << uri->get_host() << " timeout:" << timeout << "]";
    ...
    if (!socket)
    {
        ...
        ss << "[socket create fail"
           << " address:" << addr << " errno:" << strerror(errno) << " timeout:" << timeout << "]";
...
HTTPConnectionPool::ptr HTTPConnectionPool::Create(const std::string &uri_str, const std::string &vhost,
                                                   uint32_t max_size, uint32_t max_alive_time, uint32_t max_request)
{
    ...
    return std::make_shared<HTTPConnectionPool>(uri->get_host(), vhost, uri->get_scheme() == "https", uri->get_port(),
                                                max_size, max_alive_time, max_request);
}

HTTPConnectionPool::HTTPConnectionPool(const std::string &host, const std::string &vhost, bool is_https, uint32_t port,
                                       uint32_t max_size, uint32_t max_alive_time, uint32_t max_request)
    : m_host(host), m_vhost(vhost), m_is_https(is_https), m_port(port), m_max_size(max_size),
      m_max_alive_time(max_alive_time), m_max_request(max_request)
{
}
...
HTTPResult::ptr HTTPConnectionPool::get(const std::string &path, uint64_t timeout,
                                        const std::map<std::string, std::string> &headers, const std::string &body)
{
    return request(HTTPMethod::GET, path, timeout, headers, body);
}

HTTPResult::ptr HTTPConnectionPool::get(URI::ptr uri, uint64_t timeout,
                                        const std::map<std::string, std::string> &headers, const std::string &body)
{
    std::stringstream ss;
    ss << uri->get_path() << (uri->get_query().empty() ? "" : "?") << uri->get_query()
       << (uri->get_fragment().empty() ? "" : "#") << uri->get_fragment();
    return get(ss.str(), timeout, headers, body);
}

HTTPResult::ptr HTTPConnectionPool::post(const std::string &path, uint64_t timeout,
                                         const std::map<std::string, std::string> &headers, const std::string &body)
{
...
HTTPResult::ptr HTTPConnectionPool::request(HTTPMethod method, const std::string &path, uint64_t timeout,
                                            const std::map<std::string, std::string> &headers, const std::string &body)
{
    auto req = std::make_shared<HTTPRequest>();
    req->set_path(path);
...
HTTPResult::ptr HTTPConnectionPool::request(HTTPMethod method, URI::ptr uri, uint64_t timeout,
                                            const std::map<std::string, std::string> &headers, const std::string &body)
{
    std::stringstream ss;
    ss << uri->get_path() << (uri->get_query().empty() ? "" : "?") << uri->get_query()
...
HTTPResult::ptr HTTPConnectionPool::request(HTTPRequest::ptr req, uint64_t timeout)
{
    ...
    if (!conn)
    {
        ...
        ss << "[get connection from pool fail"
           << " req:" << req->to_string() << " timeout: " << timeout << "]";
    ...
    if (!socket)
    {
        ...
        ss << "[connection invalid"
           << " host:" << m_host << " port:" << m_port << " timeout:" << timeout << "]";
    ...
    if (rt == 0)
    {
        std::stringstream ss;
        ss << "[socket closed"
           << " remote:" << socket->get_remote_addr() << " timeout:" << timeout << "]";
        return std::make_shared<HTTPResult>(nullptr, HTTPResult::ErrorCode::SEND_CLOSE_BY_PEER, ss.str());
    }
    if (rt < 0)
    {
...
{
    ++ptr->m_request;
    if (!ptr->is_connected() || (ptr->m_create_time + pool->m_max_alive_time) <= (uint64_t)Millisecond() ||
        ptr->m_request >= pool->m_max_request)
    {
        TIGER_LOG_D(SYSTEM_LOG) << "[connected:" << ptr->is_connected() << " creatTime:" << ptr->m_create_time
                                << " aliveTime:" << pool->m_max_alive_time << " request:" << ptr->m_request
                                << " maxRequest" << pool->m_max_request << " total:" << pool->m_total << "]";
        delete ptr;
        --pool->m_total;
        return;
    }
