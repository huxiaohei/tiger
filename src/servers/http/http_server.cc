...
HTTPServer::HTTPServer(bool keepalive, IOManager::ptr worker, IOManager::ptr accept, const std::string &name)
    : TCPServer(worker, accept, name), m_keepalive(keepalive), m_dispatch(std::make_shared<ServletDispatch>())
{
}
        ...
        if (!req)
        {
            ...
            else
            {
                TIGER_LOG_W(SYSTEM_LOG) << "[recv fail"
                                        << " localAddr:" << client->get_local_addr()
                                        << " remoteAddr:" << client->get_remote_addr() << " errno:" << strerror(errno)
                                        << "]";
            }
            break;
        }
