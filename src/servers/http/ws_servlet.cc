...
FunctionWSServlet::FunctionWSServlet(HandleCallback cb,
...
int32_t FunctionWSServlet::on_connect(HTTPRequest::ptr header,
...
int32_t FunctionWSServlet::on_close(HTTPRequest::ptr header,
                                    WSSession::ptr session) {
    if (m_close) {
...
WSServletDispatch::WSServletDispatch(const std::string &name)
    : ServletDispatch(name) {
}

void WSServletDispatch::add_servlet(const std::string &url,
                                    FunctionWSServlet::HandleCallback cb,
                                    FunctionWSServlet::ConnectCallback connect_cb,
                                    FunctionWSServlet::CloseCallback close_cb) {
    ServletDispatch::add_servlet(url,
                                 std::make_shared<FunctionWSServlet>(cb,
                                                                     connect_cb,
                                                                     close_cb));
}
...
void WSServletDispatch::add_glob_servlet(const std::string &url,
                                         FunctionWSServlet::HandleCallback cb,
                                         FunctionWSServlet::ConnectCallback connect_cb,
                                         FunctionWSServlet::CloseCallback close_cb) {
    ServletDispatch::add_glob_servlet(url,
                                      std::make_shared<FunctionWSServlet>(cb,
                                                                          connect_cb,
                                                                          close_cb));
}
