...
static tiger::ConfigVar<uint32_t>::ptr g_websocket_message_max_size = tiger::Config::Lookup<uint32_t>("tiger.websocket.msgMaxSize", (uint32_t)(1024 * 1024 * 32), "websocket message max size");
static tiger::ConfigVar<std::string>::ptr g_websocket_sec_key = tiger::Config::Lookup<std::string>("tiger.websocket.secKey", "abcdefghijklmnop", "websocket sec key");

WSConnection::WSConnection(Socket::ptr socket, bool owner)
    : HTTPConnection(socket, owner) {
}
...
std::pair<HTTPResult::ptr, WSConnection::ptr> WSConnection::Create(URI::ptr uri,
    ...
    if (!addr) {
        ...
        return std::make_pair(
            std::make_shared<HTTPResult>(nullptr,
    ...
    if (rsp->get_status() != HTTPStatus::SWITCHING_PROTOCOLS) {
        return std::make_pair(
            std::make_shared<HTTPResult>(nullptr,
                                         HTTPResult::ErrorCode::INVALID_PROTOCOLS,
                                         "not websocket server addr:" + addr->to_string()),
            nullptr);
    }
    return std::make_pair(
        std::make_shared<HTTPResult>(rsp,
