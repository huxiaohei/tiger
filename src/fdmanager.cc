...
static ConfigVar<uint64_t>::ptr g_tcp_connect_timeout =
    Config::Lookup<uint64_t>("tiger.tcp.connectTimeout", 6000, "tcp connect timeout");
static ConfigVar<uint64_t>::ptr g_tcp_recv_timeout =
    Config::Lookup<uint64_t>("tiger.tcp.recvTimeout", 6000, "tcp recv timeout");
static ConfigVar<uint64_t>::ptr g_tcp_send_timeout =
    Config::Lookup<uint64_t>("tiger.tcp.sendTimeout", 6000, "tcp send timeout");

FDEntity::FDEntity(int fd) : m_fd(fd)
{
    init();
}
