...
template <> class LexicalCast<std::string, LoggerDefine>
{
  public:
...
template <> class LexicalCast<LoggerDefine, std::string>
{
  public:
...
{
  public:
    StringFormatItem(const std::string &fmt = "") : LogFormatter::FormatItem(fmt)
    {
    }
    void format(const std::shared_ptr<Logger> logger, std::ostream &os, LogLevel::Level level, LogEvent::ptr event)
    {
        os << m_fmt;
    }
};
...
{
  public:
    DateTimeFormatItem(const std::string &fmt = "%Y-%m-%d %H:%M:%S") : LogFormatter::FormatItem(fmt)
    {
    }
    void format(const std::shared_ptr<Logger> logger, std::ostream &os, LogLevel::Level level, LogEvent::ptr event)
    {
...
{
  public:
    TabFormatItem(const std::string &fmt = "") : LogFormatter::FormatItem(fmt)
    {
    }
    void format(const std::shared_ptr<Logger> logger, std::ostream &os, LogLevel::Level level, LogEvent::ptr event)
    {
...
{
  public:
    CoIdFormatItem(const std::string &fmt = "") : LogFormatter::FormatItem(fmt)
    {
    }
    void format(const std::shared_ptr<Logger> logger, std::ostream &os, LogLevel::Level level, LogEvent::ptr event)
    {
...
{
  public:
    ThreadNameFormatItem(const std::string &fmt = "") : LogFormatter::FormatItem(fmt)
    {
    }
    void format(const std::shared_ptr<Logger> logger, std::ostream &os, LogLevel::Level level, LogEvent::ptr event)
    {
...
std::ostream &LogFormatter::format(std::shared_ptr<Logger> logger, std::ostream &ofs, LogLevel::Level level,
                                   LogEvent::ptr event)
{
    for (auto &i : m_items)
...
FileLogAppender::FileLogAppender(LogFormatter::ptr formatter, LogLevel::Level level, const std::string &path,
                                 size_t interval)
...
LoggerMgr::LoggerMgr()
{
    ...
    auto std_out_appender = std::make_shared<tiger::StdOutLogAppender>(formatter, tiger::LogLevel::DEBUG);
...
{
    ConfigVar<std::vector<LoggerDefine>>::ptr cfg =
        Config::Lookup(name + ".log", (std::vector<LoggerDefine>){}, "__logs__");
