...
TimerManager::Timer::Timer(time_t ms, bool loop, std::function<void()> cb) : m_interval(ms), m_loop(loop)
{
    m_id = ++s_timer_id;
...
bool TimerManager::Timer::Comparator::operator()(const Timer::ptr &l, const Timer::ptr &r) const
{
    ...
    if (!l)
        return false;
    if (l->m_next_time > r->m_next_time)
        return false;
    if (l->m_next_time < r->m_next_time)
        return true;
    return l.get() < r.get();
...
TimerManager::Timer::ptr TimerManager::add_cond_timer(time_t interval, std::function<void()> cb,
                                                      std::weak_ptr<void> cond, bool loop)
{
