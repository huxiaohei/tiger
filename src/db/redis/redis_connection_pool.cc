...
int RedisConnectionPool::SETBIT(const std::string &key, int offset, int val) {
    auto cmd = fmt::format("*4\r\n$6\r\nSETBIT\r\n${}\r\n{}\r\n${}\r\n{}\r\n${}\r\n{}\r\n", key.size(), key, std::to_string(offset).size(), offset, std::to_string(val), val);
    auto rst = get_connection()->exec_cmd<RedisResultVal<int>>(cmd);
    return rst->get_data();
}

int RedisConnectionPool::SETRANGE(const std::string &key, int offset, std::string &val) {
    auto cmd = fmt::format("*4\r\n$8\r\nSETRANGE\r\n${}\r\n{}\r\n${}\r\n{}\r\n${}\r\n{}\r\n", key.size(), key, std::to_string(offset).size(), offset, val.size(), val);
    auto rst = get_connection()->exec_cmd<RedisResultVal<int>>(cmd);
    return rst->get_data();
}

size_t RedisConnectionPool::APPEND(const std::string &key, std::string &val) {
    auto cmd = fmt::format("*3\r\n$6\r\nAPPEND\r\n${}\r\n{}\r\n${}\r\n{}\r\n", key.size(), key, val.size(), val);
...
long int RedisConnectionPool::INCRBY(const std::string &key, long int val) {
    auto cmd = fmt::format("*3\r\n$6\r\nINCRBY\r\n${}\r\n{}\r\n${}\r\n{}\r\n", key.size(), key, std::to_string(val), val);
    auto rst = get_connection()->exec_cmd<RedisResultVal<long int>>(cmd);
    return rst->get_data();
}
...
std::string &RedisConnectionPool::GETSET(const std::string &key, const std::string &val) {
    auto cmd = fmt::format("*3\r\n$6\r\nGETSET\r\n${}\r\n{}\r\n${}\r\n{}\r\n", key.size(), key, val.size(), val);
    auto rst = get_connection()->exec_cmd<RedisResultVal<std::string>>(cmd);
    return rst->get_data();
}

std::string &RedisConnectionPool::GET(const std::string &key) {
    auto cmd = fmt::format("*2\r\n$3\r\nGET\r\n${}\r\n{}\r\n", key.size(), key);
    auto rst = get_connection()->exec_cmd<RedisResultVal<std::string>>(cmd);
    return rst->get_data();
}
