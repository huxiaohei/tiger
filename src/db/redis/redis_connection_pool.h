...
class RedisConnectionPool {
   ...
   public:
    ...
    template <typename T>
    std::pair<int64_t, std::vector<T>> HSCAN(const std::string &zset, int64_t cursor, const std::string &pattern = "*", int64_t count = 10) {
        ...
        auto rst = get_connection()->exec_cmd<RedisResultScan<T>>(cmd);
        return rst->get_data();
    }
    /********************************************************** Hash **********************************************************/
    ...
    std::vector<std::string> ZRANGEBYSCORE(const std::string &zset, int64_t min, int64_t max, bool with_score = false);
    std::vector<std::string> ZRANGEBYSCORE(const std::string &zset, int64_t min, int64_t max, size_t offset, size_t count, bool with_score = false);
    std::vector<std::string> ZREVRANGEBYSCORE(const std::string &zset, int64_t max, int64_t min, bool with_score = false);
    std::vector<std::string> ZREVRANGEBYSCORE(const std::string &zset, int64_t max, int64_t min, size_t offset, size_t count, bool with_score = false);
    size_t ZRANK(const std::string &zset, const std::string &member);
    size_t ZREM(const std::string &zset, size_t n, ...);
    size_t ZREMRANGEBYLEX(const std::string &zset, const std::string &min, const std::string &max);
    ...
    template <typename T>
    std::pair<int64_t, std::vector<T>> ZSCAN(const std::string &zset, int64_t cursor, const std::string &pattern = "*", int64_t count = 10) {
        std::string cmd = fmt::format("*7\r\n$5\r\nZSCAN\r\n${}\r\n{}\r\n${}\r\n{}\r\n$5\r\nMATCH\r\n${}\r\n{}\r\n$5\r\nCOUNT\r\n${}\r\n{}\r\n", zset.size(), zset, std::to_string(cursor).size(), cursor, pattern.size(), pattern, std::to_string(count).size(), count);
        auto rst = get_connection()->exec_cmd<RedisResultScan<T>>(cmd);
        return rst->get_data();
    }
