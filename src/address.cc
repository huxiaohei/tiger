...
bool Address::Lookup(std::vector<Address::ptr> &r, const std::string &host, int family, int type, int protocol)
{
    ...
    if (err)
    {
        TIGER_LOG_E(SYSTEM_LOG) << "[getaddeinfo faills "
                                << " host:" << host << " family:" << family << " type:" << type
                                << " protocol:" << protocol << " error:" << gai_strerror(err) << "]";
        return false;
    }
    next = results;
    while (next)
...
IPAddress::ptr IPAddress::LookupAny(const std::string &host, int family, int type, int protocol)
{
...
bool IPAddress::InterfaceAddresses(std::multimap<std::string, std::pair<IPAddress::ptr, uint32_t>> &r, int family)
{
...
bool IPAddress::InterfaceAddresses(std::vector<std::pair<IPAddress::ptr, uint32_t>> &r, const std::string &iface,
                                   int family)
{
...
bool Address::operator==(const Address &other) const
{
    return get_addr_len() == other.get_addr_len() && memcmp(get_addr(), other.get_addr(), get_addr_len()) == 0;
}
...
IPv4Address::ptr IPv4Address::Create(const char *addr, uint16_t port)
{
    ...
    if (err <= 0)
    {
        TIGER_LOG_E(SYSTEM_LOG) << "[inet_pton error"
                                << " addr:" << addr << " err:" << err << " errno:" << strerror(errno) << "]";
        return nullptr;
    }
    return rt;
}
